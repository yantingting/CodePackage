...
SELECT orderno as loanid, customerid, createtime, ruleresult as score,
...
WHERE ruleresultname in('newUserModelScoreV6')
...
select orderno as loanid, customerid, createtime, substring(ruleresult,1,3)::int as newusermodelscorev6,
outputmap::json ->> 'newUserModelScoreV6InputModelParams' as newUserModelScoreV6InputModelParams
from rt_risk_mongo_gocash_riskreport
where createtime between '{{start_date}}' and '{{end_date}}'
and businessid = '{{businessid}}'
and ruleresultname = 'newUserModelScoreV6'
"""
...
when paid_off_time::Date-due_date::Date>3 then 1 
when loan_status='COLLECTION' and current_date::Date-due_date::Date<=3 then -3 
when loan_status='COLLECTION' and current_date::Date-due_date::Date>3 then 1
...
case when extend_times>3 then 0 
when paid_off_time::Date-due_date::Date>7 then 1 
when loan_status='COLLECTION' and current_date::Date-due_date::Date<=7 then -3 
when loan_status='COLLECTION' and current_date::Date-due_date::Date>7 then 1
when extend_times<=3 and extend_times>0 and loan_status='FUNDED' then -2
when current_date-effective_date < approved_period and loan_status!='ADVANCE_PAIDOFF' then -1
else 0 end as flag7
