...
INNER JOIN (SELECT id as loanid, customerid,createtime,ruleresult,datasources, orderno
           ...
           WHERE ruleresultname in('oldUserModelV2Score') and businessid='uku' 
...
FROM (SELECT id as loan_id
    FROM rt_t_gocash_core_loan
    WHERE return_flag = 'true' and apply_time between '{{start_date}}' and '{{end_date}}' and customer_type = 'ONE_KEY_LOAN' 
    and product_id = 1) t1
INNER JOIN (SELECT id as loanid, customerid,createtime,ruleresult,datasources, orderno
           FROM rt_risk_mongo_gocash_riskreport r
           WHERE ruleresultname in('oldUserModelV2Score') and businessid='uku' 
...
FROM (SELECT id as loan_id
    FROM rt_t_gocash_core_loan
    WHERE return_flag = 'true' and apply_time between '{{start_date}}' and '{{end_date}}' and customer_type = 'MODIFY' 
    and product_id = 1) t1
INNER JOIN (SELECT id as loanid, customerid,createtime,ruleresult,datasources, orderno
           FROM rt_risk_mongo_gocash_riskreport r
           WHERE ruleresultname in('oldUserModelV2Score') and businessid='uku' 
...
WHERE ruleresultname in ('oldUserModelV2Score')
)
SELECT *
FROM perf
...
SELECT id as loan_id
    ...
    , case when late_days >= 7 then 1 else 0 end as ever7  
FROM dw_gocash_go_cash_loan_gocash_core_loan
WHERE effective_date!='1970-01-01' and effective_date between cast('{{ start_date }}':: timestamp as date)  and cast('{{ end_date }}':: timestamp as date) and customer_type = 'ONE_KEY_LOAN' 
and current_date :: Date - due_date >=1
),
score as (
SELECT orderno
     , customerid
     ...
     , ruleresult as score
FROM rt_risk_mongo_gocash_riskreport
WHERE ruleresultname in ('oldUserModelV2Score','alwaysPass')
)
SELECT *
FROM perf
LEFT JOIN score on perf.loan_id :: varchar = score.orderno
"""
...
FROM dw_gocash_go_cash_loan_gocash_core_loan
WHERE effective_date!='1970-01-01' and effective_date between cast('{{ start_date }}':: timestamp as date)  and cast('{{ end_date }}':: timestamp as date) 
and customer_type != 'ONE_KEY_LOAN'  --and current_date :: Date - due_date >=1
),
score as (
SELECT orderno
     , customerid
