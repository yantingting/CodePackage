...
class XGBModel(object):
    ...
    def run(self, params_range=None, param_experience=None, xgb_select=True):
        ...
        self.get_params(params_range, param_experience)
    ...
    def get_params(self, params_range=None, param_experience=None):
        # 经验
        if 'XGBExp' in self.params_selection_method:
    ...
    def xgboost_cv(self, X, y, param, folds=5):
        ...
        p_test_list = []
        for train_index , test_index in kf.split(X):
            _, xgbmodel, _ = self.xgboost(X.iloc[train_index], y.iloc[train_index],
                                            param)
