...
class BinWoe(object):
    ...
    def convert_xgboost_rebins_to_dummy(self,X_derived,X_cat,dummy_var_name_map,is_apply=False):
        ...
        if not is_apply:
            ...
            dummy_var_name_map.update(bin_dummy_map)
        else:
            X_cat_dummy = pd.get_dummies(X_cat)
            X_cat_dummy.rename(columns=dummy_var_name_map,inplace=True)
...
class Performance(object):
    ...
    def calculate_score(self, coeficients, data, job, var_dict=None, \
                        manual_adjust_var_bin=None, point_double_odds=20, \
                        base_odds=1, base_points=600):
        """
        根据job的不同计算总分，或是评分卡。
