...
from sklearn.linear_model import LassoCV, Lasso, LogisticRegression,LinearRegression
...
class FeatureSelection(object):
    ...
    def xgboost(self, X, y, grid_search=False, param=None):
        ...
        importances_dict = bst.get_score(importance_type='total_gain')
    ...
    def cal_slope(self, train, test):
        """
        通过把计算X与Y的univariate chart的斜率，选择特征
        分别计算训练集和测试集的斜率，如果斜率 同正或同负 保留改特征，否则舍弃
        """
        slope_dict = dict()
        train['slope'] = 0
        test['slope'] = 1
        ...
        for feature in train.columns:
            ...
            bad_rate_1 = sub1.groupby("bin").label.mean()
            clf = LinearRegression()
            clf.fit(bad_rate_1.index.values.reshape(-1,1), list(bad_rate_1.values))
            coef_1 = clf.coef_[0]

            bad_rate_2 = sub2.groupby("bin").label.mean()
            clf = LinearRegression()
            clf.fit(bad_rate_2.index.values.reshape(-1,1), list(bad_rate_2.values))
            coef_2 = clf.coef_[0]

            if coef_1*coef_2 >= 0:
                slope_dict[feature] = np.sqrt(coef_1*coef_2)
            else:
                slope_dict[feature] = -np.sqrt(np.abs(coef_1*coef_2))

        train = train.drop(['slope'], axis=1)
        test = test.drop(['slope'], axis=1)
        del slope_dict['slope']
        del slope_dict['label']
        return slope_dict
...
class Colinearity(object):
    ...
    def calculate_corr(self, ranking_result, corr_threshold=0.7):
        ...
        return self.corr_exclude_vars, self.final_corr_df
...
def feature_remove(train, test, ranking_result, RESULT_PATH, psi = -1, iv = -1, imp = -1, corr =1 , slope = False):
    ...
    if corr < 1:
        cl = Colinearity(X_train, RESULT_PATH)
        ...
        #print(u"相关性特征选择的个数{}".format(len(corr_select)), corr_select)
        print(u"相关性特征筛掉的个数{}".format(len(corr_remove)), corr_remove)
        f_rmv = list(set(f_rmv + corr_remove))

    if slope:
        slope_dict = FeatureSelection().cal_slope(train, test)
        slope_select = [i[0] for i in slope_dict.items() if i[1] >= 0]
        '''
        try:
            slope_select.remove('slope')
            slope_select.remove('label')
        except:
            pass
        '''
        slope_remove = list(set(f_in_model)- set(slope_select))
        #print(u"univariate斜率特征选择的个数{}".format(len(slope_select)))
        print(u"univariate斜率特征筛掉的个数{}".format(len(slope_remove)), slope_remove)
        f_rmv = list(set(f_rmv + slope_remove))

    print(u"总共筛掉的个数{}".format(len(f_rmv)))

    f_used = list(set(f_in_model) - set(f_rmv))
    print(u"剩余变量个数{}".format(len(f_used)), f_used)
