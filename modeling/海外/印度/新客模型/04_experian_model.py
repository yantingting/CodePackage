...
"""
@File    : 04_experian_model.py
@Time    : 2020-07-01 11:11
@Author  : yantingting
...
result_path = '/Users/yantingting/Seafile/风控/模型/15 印度/V4模型/02_experian'
if not os.path.exists(result_path):
    os.makedirs(result_path)

# '''读入数据'''
x_with_y = pd.read_excel(os.path.join(data_path,'date_Experian.xlsx'))
x_with_y.head()
x_with_y.shape
...
# """字典"""
# var_dict 里只能存放备选变量
var_dict = pd.read_excel(os.path.join(data_path,'dict_experian.xlsx'))
y_cols = list(set(x_with_y.columns) - set(var_dict.指标英文))
x_col = list(set(x_with_y.columns).intersection(var_dict.指标英文))
useless_vars = ['BureauScore', 'BureauScoreConfidLevel']

# """EDA"""
# 删除异常值比例较高的变量
X = x_with_y[x_col]
eda_result = ss.eda(X = X, var_dict=var_dict, useless_vars =useless_vars,exempt_vars = [], uniqvalue_cutoff=0.9)
...
#     try:
#         pt.univariate_chart(x_with_y,cols,'label',sample_set='sample_set',n_bins = 10,default_value = -1,draw_all = True,\
#                      draw_list = ['train', 'test','oot'],result_path = result_path)
#     except IndexError:
...
time1 = time.time()
train_df.head()
f_rmv = fs.feature_remove(train,test, ranking_result, result_path, psi = -1, iv = 0.005,
                          imp = 0, corr = 0.8, slope = True)
