...
'''###################### 取数 ######################'''
sql_loan = '''
select l.id::text as loan_id, effective_date, a.newusermodelv4score, a.newusermodelv4result
from dw_gocash_go_cash_loan_gocash_core_loan l
left join risk_gocash_mongo_riskcontrolresult a on l.id = a.loanid::bigint
where l.return_flag = 'false' and l.effective_date between '2019-11-07' and '2019-12-23'
'''
conn.rollback()
cur = conn.cursor()
cur.execute(sql_loan)
r_loan = pd.DataFrame(cur.fetchall(),  columns = [elt[0] for elt in cur.description])
print(r_loan.shape)
print(r_loan.loan_id.nunique())
print(r_loan.dtypes)
...
sql_izi1 = '''
WITH loan as 
(select id::text as loan_id, apply_time, customer_id
from dw_gocash_go_cash_loan_gocash_core_loan l
where l.return_flag = 'false' and l.effective_date between '2019-11-07' and '2019-12-23'
),
izi as (
...
where a.return_flag = 'false' and a.effective_date between '2019-11-07' and '2019-12-23'
) t 
where rn = 1
'''
conn.rollback()
...
WHERE return_flag = 'false' and effective_date between  '2019-11-07' and '2019-12-23' ) a
...
data_apply = r_loan.merge(r_izi1, on = 'loan_id', how = 'left')
...
# decile_v4 = pd.cut(mydata['newusermodelv4score'].astype(float), bin_v4, precision=0).astype(str)

data_scored = pd.DataFrame([mydata['loan_id'].values, 
                            mydata['effective_date'].values, 
                            mydata['newusermodelv4score'].values,  
                            mydata['newusermodelv4result'].values,
                            score, decile_score, decile_score20, ypred, decile_prob, decile_prob20]).T
data_scored.columns= ['loan_id', 'effective_date', 'newusermodelv4score', 'newusermodelv4result',
                      'score', 'decile_score', 'decile_score20', 'prob', 'decile_prob', 'decile_prob20']

print(data_scored.dtypes)
print(data_scored.shape)

writer = pd.ExcelWriter('D:\\Model Development\\201912 IDN new v5\\03 Result\\tmp.xlsx')
data_scored.to_excel(writer, 'data_scored')
writer.save()


v4_score_1107_1223 = pd.read_csv('D:\\Model Development\\201912 IDN new v5\\03 Result\\v4_score_1107_1223.csv')
v4_score_1107_1223.loan_id = v4_score_1107_1223.loan_id.astype(str)

decile_v4 = pd.cut(v4_score_1107_1223['new_user_model_v4_score'].astype(float), bin_v4, precision=0).astype(str)
v4_score_1107_1223_bin = pd.DataFrame([v4_score_1107_1223['loan_id'].values, v4_score_1107_1223['new_user_model_v4_score'].values, decile_v4]).T

writer = pd.ExcelWriter('D:\\Model Development\\201912 IDN new v5\\03 Result\\tmp.xlsx')
v4_score_1107_1223_bin.to_excel(writer, 'v4_score_1107_1223_bin')
writer.save()
