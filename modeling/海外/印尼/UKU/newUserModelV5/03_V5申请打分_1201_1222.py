...
'''###################### 申请取数 ######################'''
sql_apply = '''
select distinct a.loanid as loan_id
--existsloanwithcontacts, installdangerapp, callwithbadguys, blacklist, invalidage, deniedbyoccupation, installspecifyapp
,par.gender
,par.age
,par.iziphoneageage
from risk_gocash_mongo_riskcontrolresult a 
left join risk_mongo_gocash_installmentriskcontrolparams par on a.rc1riskno = par.riskno
...
WITH loan as 
(
select distinct a.loanid as loan_id, a.customerid::bigint as customer_id, a.createtime as create_time
from risk_gocash_mongo_riskcontrolresult a 
where a.businessid = 'uku' and existsloanwithcontacts <> 'reject' and installdangerapp <> 'reject' and callwithbadguys <> 'reject' and blacklist <> 'reject' and invalidage <> 'reject'
and deniedbyoccupation  <> 'reject' and installspecifyapp  <> 'reject' 
and a.createtime::date between '2019-12-01' and '2019-12-22'
...
FROM (SELECT loan.loan_id
      ...
      WHERE loan.create_time::timestamp + '8 hour' >= izi.createtime::timestamp
) t
WHERE rn = 1
'''
conn.rollback()
...
FROM (SELECT apply.customer_id
      ...
      WHERE apply.create_time:: timestamp + '8 hour'  >= refer.create_time) t
WHERE rn = 1
GROUP BY loan_id
'''
conn.rollback()
...
FROM (SELECT loan.loan_id
      ...
      WHERE loan.create_time::timestamp >= bank.update_time
) t
WHERE rn = 1
'''
conn.rollback()
...
from (
select a.loanid as loan_id, b.*, row_number() over(partition by a.loanid order by b.create_time desc) as rn 
from risk_gocash_mongo_riskcontrolresult a 
left join ( 
	...
	where device_info <>'' ) b on a.customerid = b.customer_id and a.createtime::timestamp >= b.create_time
where a.businessid = 'uku' and existsloanwithcontacts <> 'reject' and installdangerapp <> 'reject' and callwithbadguys <> 'reject' and blacklist <> 'reject' and invalidage <> 'reject'
and deniedbyoccupation  <> 'reject' and installspecifyapp  <> 'reject' 
and a.createtime::date between '2019-12-01' and '2019-12-22'
) t 
...
from (SELECT loanid as loan_id, customerid as customer_id, createtime as create_time
FROM risk_gocash_mongo_riskcontrolresult a
WHERE a.businessid = 'uku' and existsloanwithcontacts <> 'reject' and installdangerapp <> 'reject' and callwithbadguys <> 'reject' and blacklist <> 'reject' and invalidage <> 'reject'
and deniedbyoccupation  <> 'reject' and installspecifyapp  <> 'reject' 
and a.createtime::date between '2019-12-01' and '2019-12-22'
) a
left join gocash_loan_risk_program_packages b on a.customer_id = b.customer_id and a.create_time::timestamp >= b.create_time) t
where t.rn = 1) tt1 
...
data_apply = r_apply.merge(r_refer, on = 'loan_id', how = 'left')
...
data_apply = data_apply.merge(r_izi1, on = 'loan_id', how = 'left')
...
data_apply.to_csv(path_applydata + 'data_apply.csv', index=False)
...
writer = pd.ExcelWriter('D:\\Model Development\\201912 IDN new v5\\03 Result\\score_apply.xlsx')
data_scored.to_excel(writer, 'data_scored apply 1201_1222')
writer.save()
