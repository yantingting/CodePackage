...
class ModelSummary2Excel(object):
        ...
        for name, data in data_dict.items():
            if ('_picture' in name) or (isinstance(data, str)):
                ...
                sheet.pictures.add(top=sheet.range('A1').height*i,left=sheet.range('A1').width*1\
                                 ,width= 750, height=350 ,image=os.path.join(self.fig_path,data))
                #print(os.getcwd())
                #线上相对路径报错, 绝对路径ok
                #sheet.pictures.add(top=sheet.range('A1').height * i, left=sheet.range('A1').width * 1, image=os.path.join(os.getcwd(), data), #height=pic_height,
    ...
    def table_format(self, rng, data=None, autofit=True):
        ...
        if autofit:
            rng.expand().autofit()
        #表头加粗
        # sheet.range((rng.row,rng.column), (rng.row, rng.column+rng.expand().columns.count-1)).api.Font.Bold = True
        #表格颜色
        #rng.expand().color=(240, 255, 255)
        #表头颜色
