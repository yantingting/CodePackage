...
def binning_format(binning):
    ...
    binning['cum_bad'] = binning['bad'].cumsum() / binning['bad'].sum()
    binning['cum_good'] = binning['good'].cumsum() / binning['good'].sum()
    binning['bin_ks'] = abs(binning['cum_good'] - binning['cum_bad'])
    binning['KS'] = max(binning['bin_ks'])
    ...
    # return
    return binning[
        ['variable', 'bin', 'count', 'count_distr', 'good', 'bad', 'badprob','cum_good', 'cum_bad',
         'woe', 'bin_iv', 'total_iv','bin_ks', 'KS' ,'breaks',
         'is_special_values']]
...
def woebin(dt, y, x=None,
           ...
           check_cate_num=False, replace_blank=True,
